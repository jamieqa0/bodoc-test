...
|------|----------|------------|----------|
| 진입 | 홈 탭 이동 (`wait_for_home` → HOME_TAB 클릭) | - | `S4_0_Entry_HomeTab` |
| 1 | 홈 탭 네비게이션 노출 | 불필요 | `S4_1_HomeTab_Nav` |
| 2 | '보험 종합진단' 텍스트 | 불필요 | `S4_2_Home_Insurance_Diagnosis` |
| 3 | '매월 내는 보험료' 텍스트 | 불필요 | `S4_3_Home_Monthly_Premium` |
| 4 | '숨은 보험금' 텍스트 | 필요 (최대 8회) | `S4_4_Home_Hidden_Insurance` |
| 5 | '내 보험 추가' 텍스트 | 필요 (최대 8회) | `S4_5_Home_Add_Insurance_Btn` |
...
| 1 | 홈탭 진입 확인 | HOME_TAB 대기 | `S5_1_Entry_HomeTab` |
| 2 | 내 보험 종합진단 | 요소 대기 후 1회 스크롤 다운 | `S5_2_Home_Insurance_Diagnosis` |
| 3 | 매월 내는 보험료 | 요소 대기 후 1회 스크롤 다운 | `S5_3_Home_Monthly_Premium` |
| 4 | AI 고민상담소 | `scroll_until_visible` (최대 5회) | `S5_4_Home_AI_Consulting` |
| 5 | 숨은 보험금 | `scroll_until_visible` (최대 5회) | `S5_5_Home_Hidden_Insurance` |
| 6 | '이런 상황일 때' 또는 '손해사정사에게' | `scroll_until_visible` (최대 5회) | `S5_6_Home_Adjuster` |
| 7 | 최하단 도달 | `scroll_to_bottom()` (최대 12회 스와이프) | - |
| 8 | 건강정보 확인하기 | `scroll_until_visible` (최대 5회) | `S5_8_Home_HealthInfo` |
...
| 1 | 진단 탭 네비게이션 노출 | `wait_for_element(DIAGNOSIS_TAB)` | `S6_1_DiagnosisTab_Nav` |
| 2 | 보험 진단 관련 텍스트 | `"//*[contains(@text,'진단') or ...]"` | `S6_2_Diagnosis_Text_Check` |
| 3 | 중간 스크롤 | `scroll_down(2)` × 2회 | `S6_3_Scroll_Diagnosis_Mid`, `S6_4_Scroll_Diagnosis_Bottom` |
| 4 | 상단으로 스크롤 후 '내 보험료' 탭 대기 | `scroll_up(3)` + `WebDriverWait(PREMIUM_TAB)` | - |
| 5 | '내 보험료' 탭 클릭 | `click(PREMIUM_TAB)` (실패 시 좌표 `(0.75, 0.15)` 폴백) | `S6_5_Diagnosis_Premium_Tab_Click` |
| 6 | '매월 내는 보험료' 타이틀 확인 | `wait_for_element(PREMIUM_TITLE, 5초)` | - |
| 7 | 실제 보험료 금액 추출 | `val_el.text` (`"원"` 포함 요소) | `S6_7_Premium_Amount_Check` |
...
| 단계 | 설명 | 성공 조건 | 스크린샷 |
|------|------|----------|----------|
| 진입 | 상품 탭 클릭 + `[@selected='true']` 대기 | 탭 활성화 | `S7_0_Entry_ProductTab` |
| 1 | '보닥 회원만을 위한 추천 상품' 타이틀 탐색 | UiScrollable 스크롤 후 XPath 확인 (7초) | `S7_1_Product_Recommend_Title` |
...
| 단계 | 설명 | 성공 조건 | 스크린샷 |
|------|------|----------|----------|
| 진입 | 건강 탭 아이콘 클릭 + `[@selected='true']` 대기 | 탭 활성화 | `S8_0_Entry_HealthTab` |
| 1 | '건강 기록' 타이틀 탐색 | UiScrollable 스크롤 후 XPath 확인 (7초) | `S8_1_HealthInfo_Title` |
...
| 단계 | 설명 | 성공 조건 | 스크린샷 |
|------|------|----------|----------|
| 진입 | 보상 탭 아이콘 클릭 + `[@selected='true']` 대기 | 탭 활성화 | `S9_0_Entry_RewardTab` |
| 1 | 보상 관련 타이틀 탐색 ('보상 사례' 키워드 스크롤) | 아래 XPath 중 하나 (7초) | `S9_1_Reward_Case` |
...
| 단계 | 설명 | 스크린샷 |
|------|------|----------|
| 진입 | '메뉴' 버튼 클릭 (`[1]`로 첫 번째 선택) | `S10_0_Entry_AllMenu` |
| 1 | '보험' 섹션 타이틀 확인 | `S10_1_Section_Insurance` |
| 2 | '보상/청구' 섹션 타이틀 확인 | `S10_2_Section_Claim` |
| 3 | '생활/건강' 섹션 타이틀 확인 | `S10_3_Section_Life` |
| 4 | '정보' 섹션 타이틀 확인 (추가 scroll_down 3회) | `S10_4_Section_Info` |
| 5 | '고객서비스' 섹션 타이틀 확인 | `S10_5_Section_CustomerService` |
...
| 단계 | 설명 | 스크린샷 |
|------|------|----------|
| 1 | 홈 탭 진입 | `S11_0_Entry_HomeTab` |
| 2 | '내 보험 추가하기' 클릭 → 보험 연결 화면 진입 | `S11_1_Entry_Insurance_Connect` |
| 3 | '40개 연결하기' 클릭 | `S11_2_After_Connect40_Click` |
| 4 | 네이버 인증서 선택 | `S11_3_Entry_Naver_Auth` |
| 5 | '동의하고 계속하기' 클릭 | `S11_4_After_Agree_Click` |
| 6 | 마이데이터 연결 완료 대기 | `S11_5_DataConnect_Complete` |
